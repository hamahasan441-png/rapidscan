...
def helper():
        ...
        print("\t./rapidscan.py example.com --skip nuclei --skip theHarvester: Skip the 'nuclei' and 'theHarvester' tests.")
...
tool_names = [
                ...
                #6
                ["nuclei_robots","Nuclei - Checks for robots.txt & sitemap.xml.","nuclei",1],
                ...
                #27
                ["nuclei_misconfig","Nuclei - Checks for common misconfigurations.","nuclei",1],

                #28
                ["nuclei_cves_critical","Nuclei - Checks for critical severity CVEs.","nuclei",1],

                #29
                ["nuclei_exposure","Nuclei - Checks for sensitive file and data exposures.","nuclei",1],

                #30
                ["gobuster_dirs","Gobuster - Brutes Directories on the Domain.","gobuster",1],

                #31
                ["nuclei_sqli","Nuclei - Checks for SQL Injection vulnerabilities.","nuclei",1],

                #32
                ["nuclei_cves_medium","Nuclei - Checks for medium severity CVEs.","nuclei",1],

                #33
                ["xsser","XSSer - Checks for Cross-Site Scripting [XSS] Attacks.","xsser",1],

                #34
                ["nuclei_ssl","Nuclei - Performs SSL/TLS vulnerability checks.","nuclei",1],

                #35
                ["nuclei_takeover","Nuclei - Checks for subdomain takeover vulnerabilities.","nuclei",1],

                #36
                ["nuclei_cves_high","Nuclei - Checks for high severity CVEs.","nuclei",1],

                #37
                ["subfinder","Subfinder - Discovers subdomains of the target domain.","subfinder",1],
                ...
                #40
                ["nuclei_tokens","Nuclei - Checks for exposed tokens and credentials.","nuclei",1],

                #41
                ["subfinder_subdomains","Subfinder - Passively discovers subdomains.","subfinder",1],
                ...
                #46
                ["nuclei_tech","Nuclei - Fingerprints technologies on the domain.","nuclei",1],

                #47
                ["nuclei_file_exposure","Nuclei - Checks for sensitive file exposures.","nuclei",1],

                #48
                ["nuclei_fuzzing","Nuclei - Fuzzes for hidden endpoints and directories.","nuclei",1],

                #49
                ["nuclei_dos","Nuclei - Checks for Denial of Service vulnerabilities.","nuclei",1],

                #50
                ["nuclei_lfi","Nuclei - Checks for LFI, RFI and RCE vulnerabilities.","nuclei",1],

                #51
                ["nuclei_xss","Nuclei - Checks for XSS, SQLi, BSQLi & Other Checks.","nuclei",1],
                ...
                #82
                ["nuclei_default","Nuclei - Comprehensive default template scan.","nuclei",1],
...
tool_cmd   = [
                ...
                #6
                ["nuclei -u http://", " -tags robots -silent -no-color"],
                ...
                #27
                ["nuclei -u http://", " -tags misconfig -silent -no-color"],

                #28
                ["nuclei -u http://", " -tags cve -severity critical -silent -no-color"],

                #29
                ["nuclei -u http://", " -tags exposure -silent -no-color"],

                #30
                ["gobuster dir -u http://", " -w /usr/share/wordlists/dirb/common.txt -q --no-error"],

                #31
                ["nuclei -u http://", " -tags sqli -silent -no-color"],

                #32
                ["nuclei -u http://", " -tags cve -severity medium -silent -no-color"],

                #33
                ["xsser --all=http://",""],

                #34
                ["nuclei -u http://", " -tags ssl -silent -no-color"],

                #35
                ["nuclei -u http://", " -tags takeover -silent -no-color"],

                #36
                ["nuclei -u http://", " -tags cve -severity high -silent -no-color"],
                ...
                #40
                ["nuclei -u http://", " -tags token -silent -no-color"],
                ...
                #46
                ["nuclei -u http://", " -tags tech -silent -no-color"],

                #47
                ["nuclei -u http://", " -tags exposure,file -silent -no-color"],

                #48
                ["nuclei -u http://", " -tags fuzz -silent -no-color"],

                #49
                ["nuclei -u http://", " -tags dos -silent -no-color"],

                #50
                ["nuclei -u http://", " -tags lfi -silent -no-color"],

                #51
                ["nuclei -u http://", " -tags xss -silent -no-color"],
                ...
                #82
                ["nuclei -u http://", " -as -silent -no-color"],
...
tool_resp   = [
                ...
                #6
                ["robots.txt/sitemap.xml exposure detected by Nuclei.","i",6],
                ...
                #27
                ["Nuclei detected misconfigurations on the target.","m",30],

                #28
                ["Nuclei found critical severity CVE vulnerabilities.","c",30],

                #29
                ["Nuclei found sensitive file or data exposures.","m",25],

                #30
                ["Open Directories Found with Gobuster.","m",26],

                #31
                ["Nuclei detected potential SQL Injection vulnerabilities.","h",39],

                #32
                ["Nuclei found medium severity CVE vulnerabilities.","m",30],

                #33
                ["XSSer found XSS vulnerabilities.","c",28],

                #34
                ["Nuclei found SSL/TLS related vulnerabilities.","m",29],

                #35
                ["Nuclei detected potential subdomain takeover.","h",52],

                #36
                ["Nuclei found high severity CVE vulnerabilities.","h",30],

                #37
                ["Found Subdomains with Subfinder.","m",31],
                ...
                #40
                ["Nuclei found exposed tokens or credentials.","c",53],

                #41
                ["Subdomains discovered with Subfinder.","m",31],
                ...
                #46
                ["Nuclei identified technologies through fingerprinting.","l",36],

                #47
                ["Nuclei detected sensitive file exposures.","m",25],

                #48
                ["Nuclei fuzz scan discovered hidden endpoints.","m",26],

                #49
                ["Nuclei detected Denial of Service vulnerabilities.","h",37],

                #50
                ["Nuclei detected possible LFI, RFI or RCE.","h",38],

                #51
                ["Nuclei detected possible XSS, SQLi, BSQLi.","h",39],
                ...
                #82
                ["Nuclei comprehensive scan found vulnerabilities.","h",30],
...
tool_status = [
                ...
                #6
                ["] [",0,proc_low," < 30s","nucleirobot",["[ERR]","Could not resolve"]],
                ...
                #27
                ["] [",0,proc_med," <  5m","nucleimisconf",["[ERR]","Could not resolve"]],

                #28
                ["] [",0,proc_high," < 10m","nucleicritcve",["[ERR]","Could not resolve"]],

                #29
                ["] [",0,proc_med," <  5m","nucleiexposure",["[ERR]","Could not resolve"]],

                #30
                ["Status:",0,proc_high," < 35m","gobuster",["Error","FATA"]],

                #31
                ["] [",0,proc_med," <  5m","nucleisqli",["[ERR]","Could not resolve"]],

                #32
                ["] [",0,proc_med," <  5m","nucleimedcve",["[ERR]","Could not resolve"]],

                #33
                ["Could not find any vulnerability!",1,proc_med," <  4m","xsser",["XSSer is not working propertly!","Could not find any vulnerability!"]],

                #34
                ["] [",0,proc_med," <  3m","nucleissl",["[ERR]","Could not resolve"]],

                #35
                ["] [",0,proc_med," <  5m","nucleitakeover",["[ERR]","Could not resolve"]],

                #36
                ["] [",0,proc_high," < 10m","nucleihighcve",["[ERR]","Could not resolve"]],

                #37
                [".",0,proc_med," <  5m","subfinder",["could not resolve","[ERR]"]],
                ...
                #40
                ["] [",0,proc_med," <  3m","nucleitokens",["[ERR]","Could not resolve"]],

                #41
                [".",0,proc_med," <  5m","subfinder2",["could not resolve","[ERR]"]],
                ...
                #46
                ["] [",0,proc_med," <  3m","nucleitech",["[ERR]","Could not resolve"]],

                #47
                ["] [",0,proc_med," <  3m","nucleifileexp",["[ERR]","Could not resolve"]],

                #48
                ["] [",0,proc_med," <  5m","nucleifuzz",["[ERR]","Could not resolve"]],

                #49
                ["] [",0,proc_med," <  5m","nucleidos",["[ERR]","Could not resolve"]],

                #50
                ["] [",0,proc_med," <  5m","nucleilfi",["[ERR]","Could not resolve"]],

                #51
                ["] [",0,proc_med," <  5m","nucleixss",["[ERR]","Could not resolve"]],
                ...
                #82
                ["] [",0,proc_high," < 30m","nucleidefault",["[ERR]","Could not resolve"]],
